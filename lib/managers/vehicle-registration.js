...
module.exports = class VehicleRegistration {
    ...
    /**
      ...
      * @return {Promise<any>}
      */
    postVehicleRegistration(vehicleregistration, tags) {
        const result = this.vault.encrypt(vehicleregistration);
        const response = this.vault.client.post(`/vault/static/${this.vault.id}/vehicleregistration`, {
    ...
    /**
         ...
         * @return {Promise<any>}
         */
    getVehicleRegistration(tokenId) {
        const response = this.vault.client.get(`/vault/static/${this.vault.id}/vehicleregistration/${tokenId}`);
        response.vehicleregistration = this.vault.decrypt(response.iv, response.authTag, response.vehicleregistration);
        return response;
    }
