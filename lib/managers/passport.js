...
module.exports = class Passport {
    ...
    /**
      ...
      * @return {Promise<any>}
      */
    postPassport(passport, tags) {
        const result = this.vault.encrypt(passport);
        const response = this.vault.client.post(`/vault/static/${this.vault.id}/passport`, {
    ...
    /**
         ...
         */
    getPassport(tokenId) {
        const response = this.vault.client.get(`/vault/static/${this.vault.id}/passport/${tokenId}`);
        response.passport = this.vault.decrypt(response.iv, response.authTag, response.passport);
        return response;
    }
