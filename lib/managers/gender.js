...
module.exports = class Gender {
    ...
    /**
      ...
      * @return {Promise<any>}
      */
    postGender(gender, tags) {
        const result = this.vault.encrypt(gender);
        const response = this.vault.client.post(`/vault/static/${this.vault.id}/gender`, {
    ...
    /**
         ...
         */
    getGender(tokenId) {
        const response = this.vault.client.get(`/vault/static/${this.vault.id}/gender/${tokenId}`);
        response.gender = this.vault.decrypt(response.iv, response.authTag, response.gender);
        return response;
    }
