...
module.exports = class DateOfBirth {
    ...
    /**
      ...
      * @return {Promise<any>}
      */
    postDateOfBirth(dateofbirth, tags) {
        const result = this.vault.encrypt(dateofbirth);
        const response = this.vault.client.post(`/vault/static/${this.vault.id}/dateofbirth`, {
    ...
    /**
         ...
         * @return {Promise<any>}
         */
    getDateOfBirth(tokenId) {
        const response = this.vault.client.get(`/vault/static/${this.vault.id}/dateofbirth/${tokenId}`);
