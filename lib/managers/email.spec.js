        ...
        it('should create a new email and return the email', async () => {
            ...
            const scope = nock(apiHost)
                .post(`/vault/communication/${vaultId}/email`, (body) => {
                    return body.emailAddress && body.iv && body.authTag && body.tags;
                })
            ...
            const email = await vault.email.postEmail(payload.emailAddress, payload.tags);
            ...
            expect(email.emailAddress).to.be.equal(payload.emailAddress);
            scope.done();
        });
    });
        ...
        it('should get an existing email', async () => {
            ...
            const scope = nock(apiHost)
                ...
                .reply(200, (uri, body, callback) => {
                    ...
                    response.emailAddress = encryptedEmail.encryptedData;
                    callback(null, response);
                });
            ...
            expect(email.iv).to.not.be.empty;
            expect(email.authTag).to.not.be.empty;
            expect(email.emailAddress).to.be.equal(emailExample);
            scope.done();
        });
    });
