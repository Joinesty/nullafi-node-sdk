...
class DateOfBirth {
    ...
    async createDateOfBirth(dateofbirth, year, month, tags) {
        if (arguments.length >= 2 && Array.isArray(year)) {
            tags = year;
            year = null;
        }
        if (arguments.length >= 2 && Array.isArray(month)) {
            tags = month;
            month = null;
        }
        if (arguments.length >= 2 && year && year.toString().length === 2) {
            month = year;
            year = null;
        }
    ...
    async retrieveDateOfBirthFromRealData(dateofbirth, tags) {
        ...
        const responseList = await this.vault.client.get(`/vault/static/${this.vault.id}/dateofbirth`, query);
