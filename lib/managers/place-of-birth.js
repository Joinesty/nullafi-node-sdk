...
module.exports = class PlaceOfBirth {
    ...
    /**
      ...
      * @return {Promise<any>}
      */
    postPlaceOfBirth(placeofbirth, tags) {
        const result = this.vault.encrypt(placeofbirth);
        const response = this.vault.client.post(`/vault/static/${this.vault.id}/placeofbirth`, {
    ...
    /**
         ...
         * @return {Promise<any>}
         */
    getPlaceOfBirth(tokenId) {
        const response = this.vault.client.get(`/vault/static/${this.vault.id}/placeofbirth/${tokenId}`);
        response.passport = this.vault.decrypt(response.iv, response.authTag, response.placeofbirth);
        return response;
    }
