...
describe('Client service', () => {
    ...
    describe('authenticate', () => {
        it('should get the sessionToken from the response', async () => {
            nock(apiHost)
                ...
                .reply(200, { token: 'some-valid-token', tenantId: '123', hashKey: '141414' });
            ...
            expect(client.tenantId).to.be.equal('123');
            expect(client.hashKey).to.be.equal('141414');
        });
    });
    ...
    describe('deleteCommunicationVault', () => {
        it('should delete an existing vault and return the ok', async () => {
            const id = 'd490157b23534215b0369a2685aab47f';
            const response = {
                ok: true,
            };

            nock(apiHost)
                .delete('/vault/communication/d490157b23534215b0369a2685aab47f')
                .reply(200, response);

            const client = new Client();
            client.hashKey = '123';
            const data = await client.deleteCommunicationVault(id);
    ...
    describe('deleteStaticVault', () => {
        it('should delete an existing vault and return the ok', async () => {
            ...
            nock(apiHost)
                .delete('/vault/static/d490157b23534215b0369a2685aab47f')
                .reply(200, response);

            const client = new Client();
            client.hashKey = '123';
            const data = await client.deleteStaticVault(id);
            expect(data).to.not.be.empty;
            expect(data.ok).to.be.true;
        });
    });
